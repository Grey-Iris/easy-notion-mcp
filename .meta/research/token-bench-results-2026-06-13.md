...
metadata raw Notion JSON carries (block UUIDs, timestamps, author objects, annotation wrappers) that a
content read does not need. Strip that metadata and the formats converge.

**Call count (modeled estimate — not measured, do not treat as a headline).** Raw `callCount` was **0 for
all 160 rows** in this capture, so call count was never actually measured. The "1 vs 98" figure quoted in
earlier drafts is a **model**: ours assumed 1, makenotion derived as `ceil(blocks/100)` recursed over the
captured block tree (see `recompute-tiers.ts`). It is also computed on **R7**, whose read is only **71.7%
content-complete** (our markdown drops real `text`) — so it pits our *incomplete* read against their
...
† **Modeled, not measured** (raw `callCount`=0); and R7's 1→98 pits our 71.7%-complete read against
makenotion's complete one — not an equal-completeness comparison. Future work: tasuku
`bench-measure-call-counts`.

The **common-IR column is the honesty control**, but it is *rough parity*, not uniform equality: on the
strictly-lossless classes it is ~1.0–1.06× (R6 1.01, R4 1.03, R3 1.06) — yet **R2 is 1.32× at 100%
completeness**, and R1 is 0.95× at 94%. So at equal information markdown is at rough parity with raw JSON,
modestly cheaper on a few clean classes, not universally equal — and the large as-consumed win is metadata
omission, not denser encoding. The common-IR factor climbs further only where our markdown drops content
(R5 3.17×, R8 1.48×) or flattens deep structure (R7 2.05×) — the same shapes the completeness scores flag.
...
- **callCount was 0 for all 160 rows** in this capture (not stored per call in `--reuse-raw` mode), so
  call count was **never measured**. Any "N calls" figure in this doc (§0, §3) is a **model** derived from
  the captured block tree (`ceil(blocks/100)` recursed, ours assumed 1), not a measurement. The block
  *structure* it is derived from (e.g. R7: 121 blocks, 96 with children) is real; the call total is
  modeled. A measured call-count run at equal completeness is future work (tasuku `bench-measure-call-counts`).
