...
function wrapUntrusted(markdown: string): string {
  return TRUST_CONTENT ? markdown : CONTENT_NOTICE + markdown;
}
    ...
    case "callout":
      return {
        ...
        callout: {
          ...
          icon: block.callout.icon ?? { type: "emoji", emoji: "💡" },
        },
      };
    case "equation":
      return {
  ...
  try {
    switch (name) {
      case "create_page": {
        ...
        const parentId = parent_page_id ?? NOTION_ROOT_PAGE_ID;
        if (!parentId) {
      ...
      case "read_page": {
        ...
        const response: Record<string, unknown> = {
          ...
          url: (page as any).url,
          markdown: wrapUntrusted(blocksToMarkdown(blocks)),
        };
      ...
      case "duplicate_page": {
        ...
        const parentId = parent_page_id ?? sourcePage.parent?.page_id ?? NOTION_ROOT_PAGE_ID;
